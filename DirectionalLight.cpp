...
// ディスクリプタサイズ
UINT DirectionalLight::sDescriptorHandleIncrementSize;
// コマンドリスト
ID3D12GraphicsCommandList* DirectionalLight::sCommandList = nullptr;
// ルートシグネチャ
ComPtr<ID3D12RootSignature> DirectionalLight::sRootSignature;
// パイプラインステートオブジェクト
ComPtr<ID3D12PipelineState> DirectionalLight::sPipelineState;
...
/// グラフィックパイプライン生成
/// </summary>
void DirectionalLight::InitializeGraphicsPipeline() {
...
//ログ
void DirectionalLight::Log(const std::string& message) {
	OutputDebugStringA(message.c_str());
}

//CompileShader
IDxcBlob* DirectionalLight::CompileShader(
...
//コンバートストリング
std::wstring DirectionalLight::ConvertString(const std::string& str) {
	if (str.empty()) {
		return std::wstring();
	}
...
std::string DirectionalLight::ConvertString(const std::wstring& str) {
	if (str.empty()) {
		return std::string();
	}
