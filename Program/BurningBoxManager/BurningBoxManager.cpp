...
void BurningBoxManager::Update(){
	...
	for (BurningBox* breakBox : BurningBoxes_) {
		if (breakBox->GetIsBreak()) {
			colliderDebugDraw_->DeleteCollider(&breakBox->GetCollider());
		}
	}
	//ブレイクフラグの立ったブロックを削除
	BurningBoxes_.remove_if([](BurningBox* breakBox) {
		if (breakBox->GetIsBreak()) {
			delete breakBox;
...
void BurningBoxManager::DrawImgui(){	
	...
	for (BurningBox* BurningBox : BurningBoxes_) {
		if (ImGui::TreeNode((std::to_string(i+1) + "個目の燃える箱").c_str())) {
...
void BurningBoxManager::AddBurningBox(TransformStructure burningBox, bool isMoving, bool isVertical){
	BurningBox* Burning = new BurningBox();
	Burning->Initialize(model_, material_, burningBox, isMoving, isVertical);

	BurningBoxes_.push_back(Burning);

	colliderDebugDraw_->AddCollider(&Burning->GetCollider());
}

void BurningBoxManager::SaveFile(const std::vector<std::string>& stages){
	//保存
	json root;
	root = json::object();
	...
	for (size_t j = 0; j < stages.size(); ++j) {
		...
		for (BurningBox* box : BurningBoxes_) {
...
void BurningBoxManager::FileOverWrite(const std::string& stage){
...
void BurningBoxManager::ChackFiles(){
...
void BurningBoxManager::LoadFiles(const std::string& stage){
...
void BurningBoxManager::LoadFile(const std::string& groupName, const std::string& stage){
	//読み込むjsonファイルのフルパスを合成する
	...
	for (const auto& i : root[groupName][stage]) {
		...
		BurningBox* box_ = new BurningBox();
		box_->Initialize(model_, material_, newTrans, isNewMove, isVerticalMove);

		BurningBoxes_.push_back(box_);

		colliderDebugDraw_->AddCollider(&box_->GetCollider());
	}
}

bool BurningBoxManager::LoadChackItem(const std::string& directoryPath, const std::string& itemName){
...
void BurningBoxManager::from_json(const json& j, Vector3& v){
